...
def test_footer_default_values():
    f = Footer(total_cnt=0, control_sum=Decimal(0))
    assert f.total_cnt == 0
...
def test_footer_validation_negative_values():
    f = Footer(total_cnt=-1, control_sum=Decimal(-5))
...
def make_sample_file():
    ...
    f = Footer(total_cnt=2, control_sum=Decimal("30.0"))
...
def test_read_line_invalid_length():
    short_line = "12345"
    from core.models_old import Transaction
    with pytest.raises(LineLengthMismatch):
